...
import { transformChatCharacters, findCharacterIdByName } from '../../src/api/characters.ts'
...
Deno.test('character hability values - should be within reasonable range', () => {
  MOCK_CHARACTERS.forEach(character => {
    assertEquals(character.hability >= 0, true, `Hability should be >= 0 for ${character.name}`)
    assertEquals(character.hability <= 100, true, `Hability should be <= 100 for ${character.name}`)
  })
})

Deno.test('character data validation - should have valid properties', () => {
  MOCK_CHARACTERS.forEach(character => {
    assertEquals(typeof character.id, 'number', `ID should be number for ${character.name}`)
    assertEquals(typeof character.name, 'string', `Name should be string for ${character.name}`)
    assertEquals(typeof character.hability, 'number', `Hability should be number for ${character.name}`)
    assertEquals(character.name.length > 0, true, `Name should not be empty for character ${character.id}`)
  })
})
...
Deno.test('character filtering by hability threshold - should filter correctly', () => {
  const highHabilityCharacters = MOCK_CHARACTERS.filter(char => char.hability >= 85)
  
  assertEquals(highHabilityCharacters.length, 3)
  assertEquals(highHabilityCharacters.some(char => char.name === 'Gandalf'), true)
  assertEquals(highHabilityCharacters.some(char => char.name === 'Legolas'), true)
  assertEquals(highHabilityCharacters.some(char => char.name === 'Aragorn'), true)
  assertEquals(highHabilityCharacters.some(char => char.name === 'Gimli'), false)
})