...
Deno.test("simulateCreatePlayerIfNotExist - should return existing user when user exists", () => {
  const result = simulateCreatePlayerIfNotExist(
    MOCK_EXISTING_USERS,
    1,
    "SomeNewName",
  );
...
Deno.test("user lookup performance - should find existing users efficiently", () => {
  ...
  const result1 = simulateCreatePlayerIfNotExist(
    MOCK_EXISTING_USERS,
    1,
    "Test",
  ) as User[];
  ...
  const result2 = simulateCreatePlayerIfNotExist(
    MOCK_EXISTING_USERS,
    2,
    "Test",
  ) as User[];
  ...
  const result3 = simulateCreatePlayerIfNotExist(
    MOCK_EXISTING_USERS,
    100,
    "Test",
  ) as User[];
