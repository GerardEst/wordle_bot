...
Deno.test("getPoints - should verify scoring formula correctness", () => {
  ...
  for (let tries = 1; tries <= 6; tries++) {
    ...
    assertEquals(
      result,
      expected,
      `Failed for ${tries} tries, expected ${expected}, got ${result}`,
    );
  }
});
...
Deno.test("getPointsForHability - should favor higher points for higher hability", () => {
  ...
  const highAverage = highHabilityResults.reduce((a, b) => a + b, 0) /
    highHabilityResults.length;
  const lowAverage = lowHabilityResults.reduce((a, b) => a + b, 0) /
    lowHabilityResults.length;
